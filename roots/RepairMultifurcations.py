...
if __name__ == "__main__":
	units = 'mm' #or 'um'
	import os
	ddir = os.getcwd()+'/4_TerminalArbors/'
	morphs = [ddir+fname for fname in os.listdir(ddir) if '.swc' in fname]
	swctool = swcToolkit()
	repairtool = MultifurcationRepairTool(units='mm')
	for morph in morphs:
		dat = swctool.load(morph)
		newdat = repairtool.iterate_all_repairs(dat)
		swctool.to_swc(arbor=newdat,target=morph.strip('.swc')+'_nomulti.swc')
		toVTK(morph.strip('.swc')+'_nomulti.swc')
	
	ddir = os.getcwd()+'/4_FullMorphs_unjittered/'
	morphs = [ddir+fname for fname in os.listdir(ddir) if '.swc' in fname]
	swctool = swcToolkit()
	repairtool = MultifurcationRepairTool()
	for morph in morphs:
