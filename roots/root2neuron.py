...
class Root2Hoc():
	...
	def arbor_to_nrn(self,arbor,labels,target=os.getcwd()+'/testmorphology.hoc',supress_points=True):
		topology_function = self.arbor_to_nrn_topology(arbor)
		if not suppress_points:
...
class Root2Py():
	...
	def arbor_to_nrn(self,arbor,labels,target=os.getcwd()+'/testmorphology.py',suppress_points=True):
		topology_function = self.arbor_to_nrn_topology(arbor)
		if not suppress_points:
			morphology_function = self.arbor_to_nrn_morphology(arbor)
			with open(target,'w') as f:
