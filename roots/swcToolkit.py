...
class swcToolkit():
	...
	def scale_point(self,a,vec):
		"""
		
		Scale a point around an origin according to user specified vector
		
		a - point to be scaled
		vector - x,y,z values to scale point by
	...
	def scale_morphology(self,arbor,vector): #unit - microns
		"""
		
		Scale a morphology around an origin according to user specified vector
		
		arbor - dictionary with branch number as keys and sections as values (each section is a list of points)
		vector - x,y,z values to scale all points by
		
		returns the scaled morphology
		
		"""
		if any(isinstance(i, list) for i in arbor[0][0]):
			for branch in arbor.keys():
				for section in arbor[branch]:
					arbor[branch][arbor[branch].index(section)] = [self.scale_point(point[:3],vector)+[point[-1]] for point in section]
					arbor[branch][arbor[branch].index(section)] = [tuple(val) for val in section]
		
		else:
			for branch in arbor.keys():
				arbor[branch]= [self.scale_point(point[:3],vector)+[point[-1]] for point in arbor[branch]]
				arbor[branch] = [tuple(val) for val in arbor[branch]]
